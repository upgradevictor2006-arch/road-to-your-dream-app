...
class PersonalAIManager:
    ...
    async def break_goal_into_periods(self, goal_title: str, goal_description: str, total_days: int, period_structure: List[Dict] = None) -> Dict[str, Any]:
        ...
        system_prompt = (
            f"{self.base_system_prompt}"
            "Твоя задача — помочь человеку разбить его цель на подпериоды, заполнив каждый период КОНКРЕТНЫМИ и ДЕТАЛЬНЫМИ задачами. "
            "\n\nПРАВИЛА:\n"
            "1. СНАЧАЛА проверь цель на выполнимость, законность и реалистичность. "
            "2. Если цель хорошая (выполнима, законна, реалистична) — для КАЖДОГО периода создай:\n"
            "   - Улучшенное название периода (конкретное, например 'Изучение основ грамматики' вместо '1-й месяц')\n"
            "   - КОНКРЕТНУЮ задачу для этого периода (что именно делать, например 'Изучить Present Simple, Past Simple, Future Simple')\n"
            "   - Подробное описание действий (как именно выполнять задачу)\n"
            "3. Задачи должны быть РАЗНЫМИ для каждого периода, логично выстраиваться в путь к цели.\n"
            "4. НЕ используй общие фразы типа 'Работа над целью' или 'Продолжение работы'.\n"
            "5. Каждая задача должна быть измеримой и конкретной.\n"
            "\nПРИМЕРЫ ХОРОШИХ ЗАДАЧ:\n"
            "- Для цели 'Выучить английский': 'Изучить базовую грамматику: времена Present, Past, Future Simple'\n"
            "- Для цели 'Прочитать книгу': 'Прочитать главы 1-3, составить конспект основных идей'\n"
            "- Для цели 'Научиться программировать': 'Изучить основы Python: переменные, циклы, функции'\n"
            "\nПРИМЕРЫ ПЛОХИХ ЗАДАЧ (НЕ ИСПОЛЬЗУЙ):\n"
            "- 'Работа над целью'\n"
            "- 'Продолжение работы'\n"
            "- 'Движение к цели'\n"
            "\nФОРМАТ ОТВЕТА:\n"
            "Верни ТОЛЬКО валидный JSON без дополнительного текста:\n"
            '{"is_serious": true, "periods": [{"id": "id_периода", "title": "конкретное название", "task": "конкретная задача", "description": "подробное описание", "days": число}], "advice": "совет"}\n'
            "ВАЖНО:\n"
            "- Все тексты на русском языке\n"
            "- Каждая задача должна быть УНИКАЛЬНОЙ и КОНКРЕТНОЙ\n"
            "- Не используй общие фразы\n"
            "- Заполни ВСЕ периоды из структуры\n"
        )
        ...
        user_prompt = (
            ...
            f"{periods_desc}\n"
            "\nИНСТРУКЦИЯ:\n"
            "1. Проверь цель: выполнима ли, законна ли, реалистична ли.\n"
            "2. Если цель хорошая — для КАЖДОГО периода из структуры создай:\n"
            "   - Конкретное название (не общее, а отражающее суть этапа)\n"
            "   - КОНКРЕТНУЮ задачу (что именно делать, не общие фразы!)\n"
            "   - Подробное описание действий\n"
            "3. Задачи должны быть РАЗНЫМИ для каждого периода и логично вести к цели.\n"
            "4. НЕ используй фразы типа 'Работа над целью' или 'Продолжение работы'.\n"
        ...
        if response:
            ...
            logger.debug(f"📝 Первые 500 символов ответа: {response[:500]}")
            ...
            try:
                ...
                if json_start >= 0 and json_end > json_start:
                    ...
                    result = json.loads(json_str)
                    logger.info(f"✅ JSON успешно распарсен. Периодов: {len(result.get('periods', []))}")
                    ...
                    if 'periods' in result and result['periods']:
                        ...
                        for period in result['periods']:
                            ...
                            if 'id' not in period:
                                logger.warning(f"⚠️ Период без id пропущен: {period}")
                                continue
                            
                            # Обрабатываем title
                            if 'title' in period and period['title']:
                                period['title'] = self._remove_english_words(period['title']).strip()
                            
                            # Обрабатываем task - это самое важное поле
                            if 'task' in period and period['task']:
                                period['task'] = self._remove_english_words(period['task']).strip()
                                # Проверяем, что task не является общим
                                if period['task'].lower() in ['работа над целью', 'продолжение работы', 'движение к цели']:
                                    logger.warning(f"⚠️ Обнаружена общая фраза в task для периода {period.get('id')}: {period['task']}")
                            elif 'description' in period and period['description']:
                                # Если task нет, используем description
                                period['task'] = self._remove_english_words(period['description']).strip()
                                logger.info(f"📝 Использован description как task для периода {period.get('id')}")
                            
                            # Обрабатываем description
                            if 'description' in period and period['description']:
                                period['description'] = self._remove_english_words(period['description']).strip()
                            
                            # Проверяем, что task не пустой
                            if not period.get('task') or not period['task'].strip():
                                logger.warning(f"⚠️ Период {period.get('id')} имеет пустой task, пропускаем")
                                continue
                            
                            processed_periods.append(period)
                            logger.debug(f"✅ Обработан период {period.get('id')}: title='{period.get('title', '')[:30]}...', task='{period.get('task', '')[:50]}...'")
                        
                        result['periods'] = processed_periods
                        logger.info(f"✅ Обработано {len(processed_periods)} периодов")
                    
                    # Проверяем, что есть хотя бы один период
                    if not result.get('periods') or len(result['periods']) == 0:
                        logger.warning("⚠️ ИИ не вернул ни одного периода, используем fallback")
                    else:
                        # Проверяем, что все периоды из структуры заполнены
                        if period_structure:
                            expected_ids = {p.get('id') for p in period_structure if p.get('id')}
                            returned_ids = {p.get('id') for p in result['periods'] if p.get('id')}
                            missing_ids = expected_ids - returned_ids
                            
                            if missing_ids:
                                logger.warning(f"⚠️ ИИ не вернул периоды для ID: {missing_ids}")
                                # Добавляем недостающие периоды из исходной структуры
                                for period in period_structure:
                                    if period.get('id') in missing_ids:
                                        # Ищем похожий период в ответе для создания задачи
                                        similar_period = None
                                        for returned_period in result['periods']:
                                            if returned_period.get('type') == period.get('type'):
                                                similar_period = returned_period
                                                break
                                        
                                        if similar_period:
                                            # Используем похожую задачу, но адаптируем
                                            task = similar_period.get('task', '')
                                            if task:
                                                task = task.replace('первый', 'следующий').replace('начальный', 'продолжающий')
                                        else:
                                            task = f"Продолжить работу над целью '{goal_title}'"
                                        
                                        result['periods'].append({
                                            "id": period.get('id'),
                                            "title": period.get('title', 'Период'),
                                            "task": task,
                                            "description": f"Выполнение задачи: {task}",
                                            "days": period.get('days', 0)
                                        })
                                        logger.info(f"➕ Добавлен недостающий период {period.get('id')} с задачей: {task}")
                        
                        logger.info(f"✅ Возвращаем {len(result['periods'])} периодов")
                        return result
                else:
                    logger.warning(f"⚠️ Не найдены фигурные скобки в ответе. Ответ: {response[:200]}")
            except json.JSONDecodeError as e:
                logger.error(f"❌ Ошибка парсинга JSON: {e}")
                logger.error(f"📝 Проблемный JSON: {json_str if 'json_str' in locals() else 'не извлечен'}")
                logger.error(f"📝 Полный ответ ИИ: {response[:1000]}")
            except Exception as e:
                logger.error(f"❌ Неожиданная ошибка при обработке ответа: {type(e).__name__}: {e}")
                logger.error(f"📝 Ответ ИИ: {response[:500]}")
        
        # Fallback: пытаемся создать более конкретные задачи на основе цели
        logger.warning("⚠️ Используем fallback - ИИ не вернул валидный ответ")
        
        if period_structure:
            # Создаем более конкретные задачи на основе типа цели
            fallback_periods = []
            total_periods = len(period_structure)
            
            for i, p in enumerate(period_structure):
                period_id = p.get('id', '')
                period_days = p.get('days', 0)
                period_type = p.get('type', 'period')
                
                # Генерируем более конкретную задачу на основе цели
                if 'английск' in goal_title.lower() or 'english' in goal_title.lower():
                    tasks = [
                        "Изучить базовую грамматику и алфавит",
                        "Практиковать произношение и базовые фразы",
                        "Изучить времена и построение предложений",
                        "Расширить словарный запас и практиковать диалоги",
                        "Читать тексты и смотреть видео на английском"
                    ]
                elif 'книг' in goal_title.lower() or 'прочита' in goal_title.lower():
                    tasks = [
                        "Выбрать книгу и составить план чтения",
                        "Читать ежедневно по установленному объему",
                        "Делать заметки и выделять ключевые идеи",
                        "Анализировать прочитанное и делать выводы",
                        "Завершить чтение и подвести итоги"
                    ]
                elif 'программир' in goal_title.lower() or 'код' in goal_title.lower():
                    tasks = [
                        "Изучить основы языка программирования",
                        "Практиковаться на простых задачах",
                        "Изучить продвинутые концепции",
                        "Создать собственный проект",
                        "Оптимизировать и улучшить проект"
                    ]
                else:
                    # Общий шаблон для других целей
                    progress = (i + 1) / total_periods
                    if progress <= 0.2:
                        task = f"Подготовка и планирование для достижения цели '{goal_title}'"
                    elif progress <= 0.5:
                        task = f"Начало работы над целью '{goal_title}'"
                    elif progress <= 0.8:
                        task = f"Основной этап работы над целью '{goal_title}'"
                    else:
                        task = f"Завершение и подведение итогов по цели '{goal_title}'"
                    tasks = [task] * total_periods
                
                # Выбираем задачу для текущего периода
                task_index = min(i, len(tasks) - 1)
                task = tasks[task_index]
                
                fallback_periods.append({
                    "id": period_id,
                    "title": p.get('title', f'Период {i+1}'),
                    "task": task,
                    "description": f"Выполнение задачи: {task}",
                    "days": period_days
                })
            
            logger.info(f"📋 Fallback: создано {len(fallback_periods)} периодов")
            return {
                "is_serious": True,
        ...
        return {
            "is_serious": True,
            "periods": [],
            "advice": "Не удалось создать периоды. Попробуйте еще раз или создайте задачи вручную."
        }
